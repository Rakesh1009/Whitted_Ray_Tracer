...
std::string scene_t::parse_string(XMLElement* _elm, std::string property)
{
	return parse_property(_elm, property, "string");
}
...
{
	return (object_t*)(new mesh_t(find_material(parse_parameter(_elm, "material"),	matlist),
									parse_vector3(_elm,  "center"),
									parse_string(_elm, "mesh_path")
									));
}
...
int scene_t::parse_materials(XMLElement* _elm) 
{
	...
	while (elm_child) 
	{
		if(strcmp(elm_child->Name(), "simplemat")==0)
			mats.push_back(parse_simplemat(elm_child));
		else if(strcmp(elm_child->Name(), "texturemat")==0)
			mats.push_back(parse_texturemat(elm_child));
...
material_t* scene_t::parse_texturemat(XMLElement* _elm) 
{
	return (material_t*)(new texturemat_t(
			parse_parameter(_elm, "id"),
			parse_string(_elm, "texture_path"),
			parse_color(_elm, "specular"),
			parse_color(_elm, "reflect"),
			parse_color(_elm, "transmit"),
			parse_double(_elm, "eta"),
